...
def main():
   ...
   """ ### EXERCISE 3 ### """
   """
   Perform a vulnerability scan of the Windows Victim machine after it's been patched.
   """
   ...
   print colored ("INFO: Starting scan of 192.168.250.17", "green")
   joval_url = "https://" + domain + "/api/1.0/apps/joval/scan?sensorId=" + sensor_uuid
   s, headers = getToken(s)
   for win in win_asset_ids:
      scan_raw = {"uuid":win}
      scan_data = json.dumps(scan_raw)
      try:
         response = s.post(joval_url, headers=headers, data=scan_data)
      except:
         print colored ("Error: Cannot access " + joval_url, "red")
         print "RAW: " + response.text
         exit()
