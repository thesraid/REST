...
def main():
   ...
   label_url = 'https://' + domain + '/api/1.0/labels'
   ...
   Create Labels
   """
   print colored ("INFO: Creating WannaCry Label", "green")
   s, headers = getToken(s)
   label_raw = {"name":"WannaCry","format":{"color":"#E56838"}}
   label_data = json.dumps(label_raw)
   try:
      response = s.post(label_url, headers=headers, data=label_data)
   except:
