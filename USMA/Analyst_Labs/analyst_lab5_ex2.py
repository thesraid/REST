...
def main():
   ...
   """ ### EXERCISE 2 ### """
   """
   Creating Forensic Reponse Rule
   """
   print colored ("INFO: Creating Forensic Reponse Rule", "green")
   s, headers = getToken(s)
   forn_raw = {"action":"sensor_app_action","actionParams":{"appName":"forensics-response","sensorUUID":sensor_uuid,"action":"shutdown","uuid":"$(source_asset_id)","params":"{\"uuid\":\"$(source_asset_id)\"}"},"uiinfo":"{\"operator\":\"and\",\"rules\":[{\"condition\":\"==\",\"field\":\"packet_type\",\"value\":\"alarm\"},{\"condition\":\"==\",\"field\":\"event_category\",\"value\":\"Malware\"},{\"condition\":\"==\",\"field\":\"event_subcategory\",\"value\":\"Ransomware\"},{\"condition\":\"==\",\"field\":\"malware_family\",\"value\":\"WannaCry\"},{\"condition\":\"!=\",\"field\":\"alarm_destinations\",\"value\":\"8.8.8.8\"}]}","matchRule":"(packet_type == 'alarm' and event_category == 'Malware' and event_subcategory == 'Ransomware' and malware_family == 'WannaCry' and alarm_destinations != '8.8.8.8')","name":"WannaCry — Emergency Shutdown","occurrences":1,"length":"1s"}
   forn_data = json.dumps(forn_raw)
   try:
      response = s.post(orchestration_url, headers=headers, data=forn_data)
   except:
